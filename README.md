...
`1/7` — the same pull as one task due a week out, in effect an extra week-away
task that never gets crossed off. It is never listed and cannot be completed, but
it competes for every draw: one task due today gives a total weight of
`2 + 1/7 = 15/7`, so that task has a `14/15` = 93.3% chance and Rest takes the
remaining 6.7%.
...
| --- | --- |
| 1 task due tomorrow | 12.5% |
| 1 task due today | 6.7% |
| 2 due today | 3.4% |
| 3 due today | 2.3% |
| a dozen mixed tasks | ~1.4% |
| 4 overdue plus a full list | <1% |
