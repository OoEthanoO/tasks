...
eq(REST_WEIGHT, 1 / 7, "rest weighs the same as a task due a week out");
eq(REST_WEIGHT, taskWeight(mk(7), today), "rest is pinned to the curve, not a literal");

const one = buildWeightTable([mk(0)], today);
eq(one.total.toFixed(6), (15 / 7).toFixed(6), "one task due today: total = 2 + 1/7 = 15/7");
...
eq(shares.map((s) => (s * 100).toFixed(1)), ["12.5", "6.7", "3.4", "2.3"], "rest share shrinks as the plate fills");
eq(
